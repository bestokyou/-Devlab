<?php
// create_worksheet.php - Page for teachers to create new worksheets
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // If no errors, save the worksheet
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO worksheets (teacher_id, title, description, language, starter_code, expected_output, difficulty, due_date)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $teacher['id'],
            ...
            $_SESSION['success_message'] = "สร้างโจทย์งานใหม่เรียบร้อยแล้ว";
...
<head>
    <title>สร้างโจทย์งานใหม่ - DevLab</title>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex justify-between items-center mb-8 pb-6 border-b bg-white shadow p-5 rounded">
            <div class="flex items-center">
                ...
                <h1 class="text-2xl font-bold">สร้างโจทย์งานใหม่</h1>
            </div>
