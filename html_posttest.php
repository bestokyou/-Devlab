...
if ($completionPercentage < 90) {
    $_SESSION['test_message'] = "คุณต้องเรียน HTML ให้ครบอย่างน้อย 90% ก่อนทำแบบทดสอบหลังเรียน";
    header('Location: dashboard_html_detail.php');
    exit();
}
...
$checkTestStmt = $pdo->prepare("
    ...
    FROM test_results 
    WHERE user_id = ? AND language = 'html' AND test_type = 'post'
");
...
if ($existingTest) {
    $_SESSION['test_message'] = "คุณได้ทำแบบทดสอบหลังเรียน HTML ไปแล้ว คะแนนของคุณคือ {$existingTest['score']}/{$existingTest['total_questions']}";
    header('Location: dashboard_html_detail.php');
    exit();
}
...
$questionsStmt = $pdo->prepare("
    ...
    WHERE language = 'html' AND type = 'post'
    ORDER BY id
");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ... (ส่วนที่เหลือของโค้ดยังเหมือนเดิม)
    ...
    try {
        ...
        $insertStmt = $pdo->prepare("
            ...
            VALUES (?, ?, ?, ?, ?)
        ");
        $insertStmt->execute([$_SESSION['user_id'], 'html', 'post', $score, $totalQuestions]);
        ...
        // แสดงผลคะแนน
        $_SESSION['test_message'] = "คุณได้ทำแบบทดสอบหลังเรียน HTML เสร็จสิ้น คะแนนของคุณคือ {$score}/{$totalQuestions}";
        
        // ไปที่หน้าแสดงผลละเอียด
        header("Location: view_test_results.php?language=html&type=post");
        exit();
    } catch (Exception $e) {
        $pdo->rollBack();
...
<head>
    <title>แบบทดสอบหลังเรียน HTML - DevLab</title>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex justify-between items-center mb-8 pb-6 border-b bg-white shadow p-5 rounded">
            <div class="flex items-center">
                ...
                <h1 class="text-2xl font-bold">แบบทดสอบหลังเรียน HTML</h1>
            </div>
            <div class="flex items-center gap-4">
                <a href="dashboard_html_detail.php" class="text-blue-500 hover:underline"> กลับไปยังหลักสูตร HTML</a>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-8">
            <div class="mb-6">
                ...
                <p class="text-gray-600">
                    แบบทดสอบนี้ประกอบด้วยข้อสอบ <?php echo count($questions); ?> ข้อ เพื่อประเมินความรู้ด้าน HTML
                    ที่คุณได้เรียนรู้
                    จากหลักสูตรนี้ คุณมีเวลาไม่จำกัดในการทำแบบทดสอบ
                </p>
