...
$checkTestStmt = $pdo->prepare("
    ...
    FROM test_results 
    WHERE user_id = ? AND language = 'php' AND test_type = 'pre'
");
...
if ($existingTest) {
    $_SESSION['test_message'] = "คุณได้ทำแบบทดสอบก่อนเรียน PHP ไปแล้ว คะแนนของคุณคือ {$existingTest['score']}/{$existingTest['total_questions']}";
    header('Location: dashboard_php_detail.php');
    exit();
}
...
$questionsStmt = $pdo->prepare("
    ...
    WHERE language = 'php' AND type = 'pre'
    ORDER BY id
");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $insertStmt = $pdo->prepare("
            ...
            VALUES (?, ?, ?, ?, ?)
        ");
        $insertStmt->execute([$_SESSION['user_id'], 'php', 'pre', $score, $totalQuestions]);
        ...
        // แสดงผลคะแนน
        $_SESSION['test_message'] = "คุณได้ทำแบบทดสอบก่อนเรียน PHP เสร็จสิ้น คะแนนของคุณคือ {$score}/{$totalQuestions}";
        
        // ไปที่หน้าแสดงผลละเอียด
        header("Location: view_test_results.php?language=php&type=pre");
        exit();
    } catch (Exception $e) {
        $pdo->rollBack();
...
<head>
    <title>แบบทดสอบก่อนเรียน PHP - DevLab</title>
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex justify-between items-center mb-8 pb-6 border-b bg-white shadow p-5 rounded">
            <div class="flex items-center">
                ...
                <h1 class="text-2xl font-bold">แบบทดสอบก่อนเรียน PHP</h1>
            </div>
            <div class="flex items-center gap-4">
                <a href="dashboard_php_detail.php" class="text-blue-500 hover:underline"> กลับไปยังหลักสูตร PHP</a>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-8">
            <div class="mb-6">
                ...
                <p class="text-gray-600">
                    แบบทดสอบนี้ประกอบด้วยข้อสอบ <?php echo count($questions); ?> ข้อ เพื่อประเมินความรู้พื้นฐานด้าน PHP
                    ของคุณก่อนเริ่มเรียนในหลักสูตรนี้ คุณมีเวลาไม่จำกัดในการทำแบบทดสอบ
                </p>
