<?php
// manage_questions.php - Admin page to manage pre/post test questions
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_question'])) {
        ...
        // First check if there are any test_answers referencing this question
...
$sql = "SELECT * FROM pre_post_tests WHERE 1=1";
$params = [];
...
$langStmt = $pdo->query("SELECT DISTINCT language FROM pre_post_tests ORDER BY language");
$languages = $langStmt->fetchAll(PDO::FETCH_COLUMN);

// If no languages found in pre_post_tests, use default languages
if (empty($languages)) {
            ...
            <form method="get" class="flex flex-wrap md:flex-nowrap gap-4">
                ...
                <div class="w-full md:w-1/4">
                    ...
                    <select name="type" class="w-full px-4 py-2 border rounded bg-gray-300">
                        ...
                        <option value="pre" <?php echo $type_filter === 'pre' ? 'selected' : ''; ?>>แบบทดสอบก่อนเรียน
                        </option>
                        <option value="post" <?php echo $type_filter === 'post' ? 'selected' : ''; ?>>แบบทดสอบหลังเรียน
                        </option>
                    </select>
                </div>
            ...
            <form method="post" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="question" required
                        class="w-full px-4 py-2 border rounded h-24 bg-gray-300"></textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-gray-700 mb-2">ตัวเลือก A</label>
                        <textarea name="option_a" required
                            class="w-full px-4 py-2 border rounded h-16 bg-gray-300"></textarea>
                    </div>

                    <div>
                        <label class="block text-gray-700 mb-2">ตัวเลือก B</label>
                        <textarea name="option_b" required
                            class="w-full px-4 py-2 border rounded h-16 bg-gray-300"></textarea>
                    </div>

                    <div>
                        <label class="block text-gray-700 mb-2">ตัวเลือก C</label>
                        <textarea name="option_c" required
                            class="w-full px-4 py-2 border rounded h-16 bg-gray-300"></textarea>
                    </div>

                    <div>
                        <label class="block text-gray-700 mb-2">ตัวเลือก D</label>
                        <textarea name="option_d" required
                            class="w-full px-4 py-2 border rounded h-16 bg-gray-300"></textarea>
                    </div>
                </div>
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex gap-2">
                                    <button
                                        class="view-question bg-gray-500 text-white px-3 py-1 rounded hover:bg-gray-600 text-sm"
                                    ...
                                    <button
                                        class="edit-question bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600 text-sm"
                                        data-id="<?php echo $question['id']; ?>"
                                        data-question="<?php echo htmlspecialchars($question['question']); ?>"
                                        data-option-a="<?php echo htmlspecialchars($question['option_a']); ?>"
                                        data-option-b="<?php echo htmlspecialchars($question['option_b']); ?>"
                                    ...
                                    <button
                                        class="delete-question bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 text-sm"
                                        data-id="<?php echo $question['id']; ?>">
                                        ลบ
                                    </button>
                                </div>
            ...
            <form method="post" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="question" id="edit_question" required
                        class="w-full px-4 py-2 border rounded h-24"></textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-gray-700 mb-2">ตัวเลือก A</label>
                        <textarea name="option_a" id="edit_option_a" required
                            class="w-full px-4 py-2 border rounded h-16"></textarea>
                    </div>

                    <div>
                        <label class="block text-gray-700 mb-2">ตัวเลือก B</label>
                        <textarea name="option_b" id="edit_option_b" required
                            class="w-full px-4 py-2 border rounded h-16"></textarea>
                    </div>

                    <div>
                        <label class="block text-gray-700 mb-2">ตัวเลือก C</label>
                        <textarea name="option_c" id="edit_option_c" required
                            class="w-full px-4 py-2 border rounded h-16"></textarea>
                    </div>

                    <div>
                        <label class="block text-gray-700 mb-2">ตัวเลือก D</label>
                        <textarea name="option_d" id="edit_option_d" required
                            class="w-full px-4 py-2 border rounded h-16"></textarea>
                    </div>
                </div>
